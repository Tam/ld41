...
public class Controller2D : RaycastController {
	...
	private const float _MAX_SLOPE_ANGLE = 59f;
	...
	private void HorizontalCollisions (ref Vector2 moveAmount)
	{
		...
		for (int i = 0; i < horizontalRayCount; i++)
		{
			...
			if (hit)
			{
				...
				if (i == 0 && slopeAngle <= _MAX_SLOPE_ANGLE)
				{
				...
				if (!collisions.climbingSlope || slopeAngle > _MAX_SLOPE_ANGLE)
				{
	...
	private void DescendSlope (ref Vector2 moveAmount)
	{
		RaycastHit2D maxSlopeHitLeft = Physics2D.Raycast(
			raycastOrigins.bottomLeft,
			Vector2.down,
			Mathf.Abs(moveAmount.y) + SKIN_WIDTH,
			collisionMask
		);
		...
		if (hit)
		{
			...
			if (slopeAngle != 0 && slopeAngle <= _MAX_SLOPE_ANGLE)
			{
	...
	private void SlideDownMaxSlope (RaycastHit2D hit, ref Vector2 moveAmount)
	{
		if (hit)
		{
			...
			if (slopeAngle > _MAX_SLOPE_ANGLE)
			{
				moveAmount.x = hit.normal.x 
							   * (Mathf.Abs(moveAmount.y) - hit.distance)
